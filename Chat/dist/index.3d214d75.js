...
let myName = "";
let myEmail = "";
myEmail = (0, _cookie.getCookie)("myEmail");
...
socket.onopen = function(e) {
    ...
    if (!code) {
        const modalAuth1 = document.querySelector(".modalAuth");
        modalAuth1.classList.add("active");
    } else {
        fetch("https://edu.strada.one/api/messages/", {
            ...
            for (let value of obj.messages)renderChat(value.user.email, value.user.name, value.text);
        }).catch((error)=>{
            console.log(error);
...
socket.onclose = function(event) {
    if (event.wasClean) alert(`[close] Соединение закрыто чисто, код=${event.code} причина=${event.reason}`);
    else // например, сервер убил процесс или сеть недоступна
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    inp.addEventListener("click", function(e) {
        ...
        (0, _cookie.setCookie)("myEmail", myEmail);
        const email = {
...
function EntCode(e) {
    ...
    const modalAuth1 = document.querySelector(".modalAuth");
    modalAuth1.classList.remove("active");
...
function renderChat(email, name, message) {
    ...
    if (email === myEmail) elem.classList.add("me");
...
},{"./cookie":"7KFrp"}],"7KFrp":[function(require,module,exports) {
...
},{}]},["8BXtR","bB7Pu"], "bB7Pu", "parcelRequire25d8")
